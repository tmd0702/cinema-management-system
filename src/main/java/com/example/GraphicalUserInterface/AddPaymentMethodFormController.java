...
public class AddPaymentMethodFormController implements Initializable {
    private ManagementMain main;
    @FXML
    private TextField nameField;
    @FXML
    private VBox addPaymentMethodForm;
    public AddPaymentMethodFormController() throws Exception {
    ...
    public void disableInsertForm() {
        ((AnchorPane)addPaymentMethodForm.getParent()).getChildren().get(0).setVisible(true);
        ((AnchorPane)addPaymentMethodForm.getParent()).getChildren().remove(2);
    }
    @FXML
    ...
    public void handleInsertRecordRequest() {
        ...
        paymentMethodInfo.put("ID", main.getIdGenerator().generateId(main.getProcessorManager().getPaymentMethodManagementProcessor().getDefaultDatabaseTable()));
        Response response = main.getProcessorManager().getPaymentMethodManagementProcessor().insertData(paymentMethodInfo, true);

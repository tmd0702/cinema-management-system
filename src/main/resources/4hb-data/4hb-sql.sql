...
DELIMITER //
CREATE TRIGGER TG_INSERT_PAYMENTS AFTER INSERT ON PAYMENTS
FOR EACH ROW
BEGIN
...
FOR EACH ROW
BEGIN
	UPDATE MOVIES SET REVENUE = REVENUE + NEW.AMOUNT WHERE ID = (SELECT MOVIE_ID
																 FROM SCHEDULES S
...
DELIMITER //
CREATE TRIGGER TG_INSERT_TICKET_ITEMS
AFTER INSERT ON TICKET_ITEMS
FOR EACH ROW
BEGIN
	UPDATE ITEMS SET REVENUE = REVENUE + NEW.AMOUNT WHERE ID = (SELECT ITEM_ID
																FROM BOOKING_ITEMS BI
                                                                WHERE BI.TICKET_ITEM_ID = NEW.ID);
END; //
DELIMITER ;
...
FOR EACH ROW
BEGIN
	DELETE FROM BOOKING_SEATS WHERE SEAT_ID = OLD.ID;
END; //
DELIMITER ;
...
FOR EACH ROW
BEGIN
	DELETE FROM BOOKING_SEATS WHERE TICKET_ID = OLD.ID;
    DELETE FROM PROMOTION_RELATIONSHIP WHERE TICKET_ID = OLD.ID;
    DELETE FROM PAYMENTS WHERE TICKET_ID = OLD.ID;
END; //
DELIMTER ;

...
public class PaymentManagementProcessor extends Processor {
    ...
    public Response getData(int from, int quantity, String queryCondition, String sortQuery) {
        if (queryCondition.length() == 0) {
            queryCondition = "USERS.ID = PAYMENTS.USER_ID AND PAYMENT_METHODS.ID = PAYMENTS.PAYMENT_METHOD_ID AND SCHEDULES.ID = PAYMENTS.SCHEDULE_ID AND SCREEN_ROOMS.ID = SCHEDULES.SCREEN_ROOM_ID AND CINEMAS.ID = SCREEN_ROOMS.CINEMA_ID AND SCHEDULES.MOVIE_ID = MOVIES.ID AND SCHEDULES.SHOW_TIME_ID = SHOW_TIMES.ID";
        } else {
            queryCondition = queryCondition + " AND USERS.ID = PAYMENTS.USER_ID AND PAYMENT_METHODS.ID = PAYMENTS.PAYMENT_METHOD_ID AND SCHEDULES.ID = PAYMENTS.SCHEDULE_ID AND SCREEN_ROOMS.ID = SCHEDULES.SCREEN_ROOM_ID AND CINEMAS.ID = SCREEN_ROOMS.CINEMA_ID AND SCHEDULES.MOVIE_ID = MOVIES.ID AND SCHEDULES.SHOW_TIME_ID = SHOW_TIMES.ID";
        }
        Response response = select("PAYMENTS.ID AS 'PAYMENTS.ID', PAYMENTS.PAYMENT_DATE AS 'PAYMENTS.PAYMENT_DATE', PAYMENT_METHODS.NAME AS 'PAYMENT_METHODS.NAME', PAYMENTS.TOTAL_AMOUNT AS 'PAYMENTS.TOTAL_AMOUNT', USERS.USERNAME AS 'USERS.USERNAME', MOVIES.TITLE AS 'MOVIES.TITLE', CINEMAS.NAME AS 'CINEMAS.NAME', SCREEN_ROOMS.NAME AS 'SCREEN_ROOMS.NAME', SCHEDULES.SHOW_DATE AS 'SCHEDULES.SHOW_DATE', SHOW_TIMES.START_TIME AS 'SHOW_TIMES.START_TIME'", from, quantity, queryCondition, sortQuery, "PAYMENTS, USERS, PAYMENT_METHODS, SCHEDULES, MOVIES, SCREEN_ROOMS, SHOW_TIMES, CINEMAS");
        return response;
    }

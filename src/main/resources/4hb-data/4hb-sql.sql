...
FOR EACH ROW
BEGIN
	SET NEW.TOTAL_AMOUNT = (SELECT AMOUNT FROM PAYMENT_TICKETS WHERE NEW.PAYMENT_TICKET_ID = ID) +
						   (SELECT AMOUNT FROM PAYMENT_ITEMS WHERE NEW.PAYMENT_ITEM_ID = ID);

	UPDATE USERS SET SCORE = (SCORE + NEW.TOTAL_AMOUNT) / 100000 WHERE ID = NEW.USER_ID;
	...
	UPDATE ITEMS SET REVENUE = REVENUE + (SELECT AMOUNT
										  FROM PAYMENT_ITEMS
                                          WHERE NEW.PAYMENT_ITEM_ID = ID)
    WHERE ID = (SELECT ITEM_ID
				FROM BOOKING_ITEMS BI
				WHERE BI.PAYMENT_ITEM_ID = NEW.ID);
...
FOR EACH ROW
BEGIN
    UPDATE MOVIES SET VOTE_AVERAGE = ((VOTE_AVERAGE * VOTE_COUNT) + NEW.RATING) / (VOTE_COUNT + 1) WHERE ID = NEW.ID;
	UPDATE MOVIES SET VOTE_COUNT = VOTE_COUNT + 1 WHERE ID = NEW.ID;
END; //
DELIMITER ;
...
FOR EACH ROW
BEGIN
    UPDATE MOVIES SET VOTE_AVERAGE = ((VOTE_AVERAGE * VOTE_COUNT) - OLD.RATING) / (VOTE_COUNT - 1) WHERE ID = OLD.ID;
	UPDATE MOVIES SET VOTE_COUNT = VOTE_COUNT - 1 WHERE ID = OLD.ID;
END; //
DELIMITER ;
...
FOR EACH ROW
BEGIN
    UPDATE MOVIES SET VOTE_AVERAGE = ((VOTE_AVERAGE * VOTE_COUNT) - OLD.RATING + NEW.RATING) / (VOTE_COUNT) WHERE ID = NEW.ID;
END; //
DELIMITER ;

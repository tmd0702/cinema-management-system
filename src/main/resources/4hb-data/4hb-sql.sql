...
BEGIN
	DECLARE score_tmp int;
    SET score_tmp = (SELECT SCORE FROM USERS WHERE ID = (SELECT USER_ID FROM PAYMENTS WHERE ID = NEW.PAYMENT_ID));
    ...
    UPDATE USERS SET SCORE = score_tmp + (SELECT PRICE FROM ITEM_PRICES WHERE ID = NEW.ITEM_PRICE_ID) * NEW.QUANTITY / 10000 WHERE ID = (SELECT USER_ID FROM PAYMENTS WHERE ID = NEW.PAYMENT_ID);
	CALL updateUserCategory((SELECT USER_ID FROM PAYMENTS WHERE ID = NEW.PAYMENT_ID));
END; //
DELIMITER ;
DELIMITER //
...
BEGIN
	...
	UPDATE USERS SET SCORE = score_tmp + (SELECT PRICE FROM SEAT_PRICES WHERE ID = NEW.SEAT_PRICE_ID) / 10000 WHERE ID = (SELECT USER_ID FROM PAYMENTS WHERE ID = NEW.PAYMENT_ID);
	CALL updateUserCategory((SELECT USER_ID FROM PAYMENTS WHERE ID = NEW.PAYMENT_ID));
END; //
DELIMITER ;
...
FOR EACH ROW
BEGIN
	UPDATE PAYMENTS SET TOTAL_AMOUNT = TOTAL_AMOUNT - (SELECT PRICE FROM SEAT_PRICES WHERE SEAT_CATEGORY_ID = (SELECT S.SEAT_CATEGORY_ID FROM SEATS S, TICKETS T WHERE S.ID = T.SEAT_ID AND T.ID = OLD.TICKET_ID)) WHERE ID = OLD.PAYMENT_ID;

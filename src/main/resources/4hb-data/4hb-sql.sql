...
BEGIN
	...
	ELSE
		...
		UPDATE ITEMS SET REVENUE = REVENUE + (SELECT SUM(P.PRICE), QUANTITY = QUANTITY - BI.QUANTITY
...
FOR EACH ROW
BEGIN
	UPDATE PAYMENT SET TOTAL_AMOUNT = TOTAL_AMOUNT - (SELECT PRICE FROM SEAT_PRICES WHERE SEAT_CATEGORY_ID = (SELECT S.SEAT_CATEGORY_ID FROM SEATS S, TICKETS T WHERE S.ID = T.SEAT_ID AND T.ID = OLD.TICKET_ID));
END; //
